...
class cervezas_model{
    ...
    public function getCerveza($id){
        ...
        $imgQuery=$this->db->prepare("SELECT file_name FROM " .$this->tabla_img. " WHERE id_cerveza=?");
    ...
    public function addCerveza($nombre,$id_estilo,$amargor,$alcohol){
        ...
        if ($imagenes!=null){
            $sentencia_img= $this->db->prepare("INSERT INTO ".$this->tabla_img." (file_name,id_cerveza) VALUES(?,?)");
    ...
    public function deleteImagen($id,$img){
        ...
        if (file_exists($arch))
             unlink($arch);
        $delete = $this->db->prepare("DELETE FROM ".$this->tabla_img." WHERE (file_name=? AND id_cerveza=?)");
